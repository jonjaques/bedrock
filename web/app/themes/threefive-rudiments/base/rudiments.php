 ...
 * @return string
 */
function rudiments_get_version( $default = '0.1' ) {

	if ( ! empty( getenv('SOURCE_VERSION') ) ) {
...
if ( ! function_exists( 'rudiments_styles_and_scripts' ) ) {
	...
	function rudiments_styles_and_scripts() {
		if (!is_admin()) {
			...
			wp_register_script( 'rudiments-modernizr', $js_dir . 'vendor/modernizr.js', array(), $version, false );
			// Rudiments custom scripts
			wp_register_script( 'rudiments-js', $js_dir . 'dist/main.js', array( 'jquery' ), $version, true );
